...
def create_impact_visualization(graph: nx.MultiDiGraph, subgraph: nx.MultiDiGraph, 
    ...
    # Show all nodes - no limiting for complete visibility
    ...
    for consolidated_id, node_info in consolidated_nodes.items():
        ...
        if node_info['is_consolidated']:
            # For consolidated nodes, show the representative ID
            tooltip = node_info['original_ids'][0]
        else:
            # Single node tooltip - just show the ID
        ...
        nodes.append(Node(
            id=consolidated_id,
            label=name[:20] + "..." if len(name) > 20 else name,  # Truncate long labels
            size=size,
            color=color,
            shape=shape,
            title=tooltip,
            font={'size': 10, 'face': 'Arial', 'color': '#000000'}  # Enhanced font for readability
        ))
    ...
    # Enhanced Configuration for better spacing - physics layout
    config = Config(
        width=2000,  # Much wider canvas
        height=1400,  # Much taller canvas  
        directed=True,
        physics=True,
        hierarchical=False,  # Use physics for natural spreading
...
def _create_consolidated_edges(subgraph: nx.MultiDiGraph, node_mapping: Dict[str, str], 
    ...
    for source, target, attrs in subgraph.edges(data=True):
        ...
        edges.append(Edge(
            ...
            width=width,
            type="CURVE_SMOOTH",
            arrows="to",  # Enhanced arrow visibility
            font={'size': 12, 'align': 'middle'}  # Better edge labels
        ))
    ...
    if len(subgraph.nodes()) > 0:
        ...
        with st.expander("🎯 Visualization Legend"):
            ...
            with col2:
                ...
                st.write("")
                st.write("**Node Sizes:**")
                st.write("🔍 Enhanced for readability (35-80px)")
                st.write("📏 Larger = More critical/selected")
                st.write("")
                st.write("**Enhanced Visibility:**")
                st.write("🖼️ Larger canvas (1400x1000)")
                st.write("🔗 Thicker edges (3-8px)")
                st.write("📝 Improved fonts (14px Arial)")
