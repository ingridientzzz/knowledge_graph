...
Uses python-igraph for high-quality graph layouts and visualization.
"""
...
# Set page config
st.set_page_config(
    page_title="dbt Impact Analysis - igraph Edition",
    page_icon="🔍",
    layout="wide",
...
def create_impact_visualization(graph: nx.MultiDiGraph, subgraph: nx.MultiDiGraph, 
                               ...
                               downstream_nodes: Set[str], layout_algorithm: str = "fr") -> go.Figure:
    """Create interactive visualization of the impact graph with consolidated sources using igraph."""
    ...
    # Convert to igraph for better layout algorithms
    ig_graph = _create_igraph_from_consolidated(consolidated_nodes, subgraph, node_mapping)
    
    # Use igraph layout algorithm 
    layout = ig_graph.layout(layout_algorithm)
    
    # Extract node positions
    node_positions = {ig_graph.vs[i]['name']: layout[i] for i in range(len(ig_graph.vs))}
    
    # Create plotly figure
    fig = go.Figure()
    
    # Add edges first (so they appear behind nodes)
    edge_traces = _create_igraph_edges(consolidated_nodes, subgraph, node_mapping, 
                                     node_positions, selected_node, upstream_nodes, downstream_nodes)
    for edge_trace in edge_traces:
        fig.add_trace(edge_trace)
    
    # Add nodes
    node_trace = _create_igraph_nodes(consolidated_nodes, node_positions, selected_node, 
                                    upstream_nodes, downstream_nodes)
    fig.add_trace(node_trace)
    
    # Update layout for better visualization
    fig.update_layout(
        title=f"Impact Analysis for: {selected_node}",
        titlefont_size=16,
        showlegend=False,
        hovermode='closest',
        margin=dict(b=20,l=5,r=5,t=40),
        annotations=[ dict(
            text="Python-igraph layout with Fruchterman-Reingold algorithm",
            showarrow=False,
...
def _create_consolidated_edges(subgraph: nx.MultiDiGraph, node_mapping: Dict[str, str], 
                             ...
                             upstream_nodes: Set[str], downstream_nodes: Set[str]) -> List[Dict[str, Any]]:
    ...
    for source, target, attrs in subgraph.edges(data=True):
        ...
        # Determine edge styling based on impact with safety checks
        if consolidated_source not in consolidated_nodes:
            continue  # Skip edges to/from nodes not in consolidated set
        ...
        if source_is_selected or target_is_selected:
            color = "#ff0000"
            width = 1  # Much thicker for selected connections
        elif source_is_upstream or target_is_downstream:
            color = NODE_TYPE_COLORS.get('upstream', '#ff9900')
            width = 2  # Thicker for important connections
        else:
            color = "#666666"
            width = 1  # Thicker base width for visibility
        
        # This function is legacy - edges are now created by _create_igraph_edges
        # Keep for compatibility but the actual edge creation happens in visualization
        edges.append({
...
def _create_igraph_from_consolidated(consolidated_nodes: Dict[str, Dict], 
                                   subgraph: nx.MultiDiGraph, 
                                   node_mapping: Dict[str, str]) -> ig.Graph:
    """Convert consolidated NetworkX graph to igraph for layout computation."""
    
    # Create igraph
    ig_graph = ig.Graph(directed=True)
    
    # Add vertices
    vertex_names = list(consolidated_nodes.keys())
    ig_graph.add_vertices(len(vertex_names))
    ig_graph.vs['name'] = vertex_names
    
    # Add node attributes
    for i, (node_id, node_info) in enumerate(consolidated_nodes.items()):
        ig_graph.vs[i]['node_type'] = node_info['node_type']
        ig_graph.vs[i]['display_name'] = node_info['name']
        ig_graph.vs[i]['is_consolidated'] = node_info['is_consolidated']
    ...
    for source, target, attrs in subgraph.edges(data=True):
        ...
        try:
            source_idx = vertex_names.index(consolidated_source)
            target_idx = vertex_names.index(consolidated_target)
            ig_graph.add_edge(source_idx, target_idx)
        except ValueError:
            continue  # Skip if vertex not found
    
    return ig_graph

def _create_igraph_nodes(consolidated_nodes: Dict[str, Dict], 
                        node_positions: Dict[str, Tuple[float, float]], 
                        selected_node: str, upstream_nodes: Set[str], 
                        downstream_nodes: Set[str]) -> go.Scatter:
    ...
    for node_id, node_info in consolidated_nodes.items():
        ...
        if selected_node in node_info['original_ids']:
            ...
            size = 15  # Other nodes - small size
        ...
        if node_info['is_consolidated']:
            hover_info = f"{name}<br>Type: {node_type}<br>Consolidated from {len(node_info['original_ids'])} packages"
        else:
            hover_info = f"{name}<br>Type: {node_type}"
...
def _create_igraph_edges(consolidated_nodes: Dict[str, Dict], 
                        subgraph: nx.MultiDiGraph, 
                        node_mapping: Dict[str, str],
                        node_positions: Dict[str, Tuple[float, float]], 
                        selected_node: str, upstream_nodes: Set[str], 
                        downstream_nodes: Set[str]) -> List[go.Scatter]:
    ...
    for source, target, attrs in subgraph.edges(data=True):
        ...
        if consolidated_source not in node_positions or consolidated_target not in node_positions:
            continue
        ...
        # Get positions
        x0, y0 = node_positions[consolidated_source]
        x1, y1 = node_positions[consolidated_target]
        
        # Determine edge styling based on impact
        if consolidated_source not in consolidated_nodes or consolidated_target not in consolidated_nodes:
            continue
        ...
        # Add arrow head
        arrow_trace = _create_arrow_head(x0, y0, x1, y1, color, width)
        edge_traces.append(arrow_trace)
    
    return edge_traces

def _create_arrow_head(x0: float, y0: float, x1: float, y1: float, 
                      color: str, width: int) -> go.Scatter:
    """Create arrow head for directed edges."""
    
    # Calculate arrow head position (90% along the edge)
    arrow_pos = 0.9
    ...
    # Arrow head size based on edge width
    arrow_size = max(0.02, width * 0.01)
    
    # Calculate arrow head points
    arrow_angle = np.pi / 6  # 30 degrees
    
    # Left point of arrow head
    left_x = arrow_x - arrow_size * (dx_norm * np.cos(arrow_angle) + dy_norm * np.sin(arrow_angle))
    left_y = arrow_y - arrow_size * (dy_norm * np.cos(arrow_angle) - dx_norm * np.sin(arrow_angle))
    
    # Right point of arrow head
    right_x = arrow_x - arrow_size * (dx_norm * np.cos(arrow_angle) - dy_norm * np.sin(arrow_angle))
    right_y = arrow_y - arrow_size * (dy_norm * np.cos(arrow_angle) + dx_norm * np.sin(arrow_angle))
    
    # Create arrow head trace
    arrow_trace = go.Scatter(
        x=[left_x, arrow_x, right_x, left_x],
        y=[left_y, arrow_y, right_y, left_y],
        mode='lines',
        fill='toself',
        fillcolor=color,
...
def main():
    """Main Streamlit app."""
    st.title("🔍 dbt Impact Analysis - igraph Edition")
    st.markdown("""
    ...
    # Removed max nodes limitation - show all connected nodes
    ...
    if len(subgraph.nodes()) > 0:
        ...
        with col2:
            layout_algorithm = st.selectbox(
                "Layout Algorithm:",
                ["fr", "kk", "drl", "lgl", "circle", "rt"],
                index=0,
                help="fr=Fruchterman-Reingold, kk=Kamada-Kawai, drl=DrL, lgl=Large Graph Layout"
            )
        
        with st.spinner("Generating igraph layout..."):
            fig = create_impact_visualization(
                graph, subgraph, selected_node, upstream_nodes, downstream_nodes, layout_algorithm
            )
        
        st.write(f"Showing impact graph with {len(subgraph.nodes())} nodes")
        
        with col1:
            # Legend
            with st.expander("🎯 Visualization Legend"):
                col1_leg, col2_leg = st.columns(2)
                with col1_leg:
                    ...
                    st.write("♦ Sources with same name = 1 node")
                    st.write("📦 Hover to see all packages")
                with col2_leg:
                    st.write("**Layout Features:**")
                    st.write("🎯 Python-igraph algorithms")
                    st.write("📐 Professional graph layouts")
                    st.write("🔗 Clear edge directions")
                    st.write("📝 Node labels displayed")
                    st.write("")
                    st.write("**Interactive Features:**")
                    st.write("🖱️ Zoom and pan")
                    st.write("ℹ️ Hover for details")
