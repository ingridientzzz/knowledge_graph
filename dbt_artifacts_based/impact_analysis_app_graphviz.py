...
PyGraphviz-based version with high-quality static graph rendering.
...
try:
    ...
    st.error("PyGraphviz is not installed. Please install it with: pip install pygraphviz")

# Set page config
st.set_page_config(
    page_title="dbt Impact Analysis - GraphViz Version",
    page_icon="🔍",
    layout="wide",
...
LAYOUT_ALGORITHMS = {
    "dot": "Hierarchical (top-to-bottom)",
    "neato": "Spring Model",
    "fdp": "Force-Directed",
...
def create_graphviz_visualization(graph: nx.MultiDiGraph, subgraph: nx.MultiDiGraph,
                                selected_node: str, upstream_nodes: Set[str], 
                                downstream_nodes: Set[str], layout_algorithm: str = "dot",
                                node_size_factor: float = 1.0, font_size: int = 10) -> str:
    """Create GraphViz visualization and return base64-encoded image."""
    ...
    G.graph_attr.update({
        'rankdir': 'TB' if layout_algorithm == 'dot' else 'LR',
        'splines': 'true',
        'overlap': 'false',
        'sep': '0.5',
        'nodesep': '0.8',
    ...
    for consolidated_id, node_info in consolidated_nodes.items():
        ...
        # Calculate size
        base_size = 0.8 * node_size_factor
        width = base_size * size_multiplier
        height = base_size * size_multiplier * 0.7
    ...
    # Add edges
    edges = _create_consolidated_edges_graphviz(subgraph, node_mapping, consolidated_nodes, 
    ...
    for edge_info in edges:
        G.add_edge(
            ...
            penwidth=edge_info['width'],
            arrowsize=edge_info.get('arrowsize', 1.0),
    ...
    with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as tmp_file:
        G.draw(tmp_file.name, format='png')
...
def _create_consolidated_edges_graphviz(subgraph: nx.MultiDiGraph, node_mapping: Dict[str, str], 
                                      consolidated_nodes: Dict[str, Dict], selected_node: str,
                                      upstream_nodes: Set[str], downstream_nodes: Set[str]) -> List[Dict]:
    """Create edges for the consolidated GraphViz visualization."""
...
def main():
    ...
    st.title("🔍 dbt Impact Analysis - GraphViz Version")
    st.markdown("**High-Quality Static Graph Rendering** | Analyze dbt dependencies with professional GraphViz layouts")
    
    if not GRAPHVIZ_AVAILABLE:
        st.error("PyGraphviz is not available. Please install it to use this version.")
        st.code("pip install pygraphviz")
        st.info("Note: On macOS, you may need to install graphviz first: `brew install graphviz`")
        return
    ...
    st.sidebar.subheader("🎨 GraphViz Layout Options")
    ...
    node_size_factor = st.sidebar.slider(
        "Node Size Factor", 0.5, 2.0, 1.0, 0.1,
    ...
    # Main analysis tabs
    tab1, tab2, tab3 = st.tabs(["📊 Impact Analysis", "🎨 GraphViz Visualization", "📋 Node Details"])
    ...
    with tab2:
        st.subheader("🎨 GraphViz Network Visualization")
        
        # Create and display the GraphViz graph
        with st.spinner(f"Creating {LAYOUT_ALGORITHMS[layout_algorithm]} visualization..."):
            try:
                img_base64 = create_graphviz_visualization(
                    graph, subgraph, selected_node, upstream_nodes, downstream_nodes,
                    layout_algorithm, node_size_factor, font_size
                )
                ...
                if img_base64:
                    ...
                    st.download_button(
                        ...
                        file_name=f"dbt_impact_graph_{selected_node.replace('.', '_')}.png",
                        mime="image/png"
                    )
                    ...
                    st.error("Failed to generate GraphViz visualization")
